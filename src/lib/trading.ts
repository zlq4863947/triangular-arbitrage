...
export class Trading {
  ...
  async testOrder(exchange: types.IExchange, triangle: types.ITriangle) {
    try {
      ...
      // 未通过检查时返回
      if (!this.check(exchange, triangle)) {
        return;
      }

      logger.info('模拟下单...');
      const freeMoney = this.balances[triangle.a.coinFrom].free;
      // 使用1%的资产购买
      const buyCapital = new BigNumber(freeMoney).times(0.01);
      if (buyCapital.isZero()) {
        logger.info(`<${triangle.a.coinFrom}>资产金额为0，不可以模拟交易！`);
        return;
      }
  ...
  // 订单执行前，可行性检查
  async check(exchange: types.IExchange, triangle: types.ITriangle): Promise<boolean> {
    if (exchange.endpoint.private || !exchange.pairs) {
      logger.info('交易所相关参数出错！！');
      return false;
    }
    ...
    const pairAFrom = triangle.a.coinFrom;
    const assetA = this.balances[pairAFrom];

    if (!assetA) {
      logger.info(`未查找到持有${pairAFrom}！！`);
      return false;
    }

    const free = new BigNumber(this.balances[pairAFrom].free);
    ...
    // 获取交易精度
    const precision = Helper.getPriceScale(exchange.pairs, pairAFrom);

    if (!precision || buyMoney.isLessThan(precision.amount)) {
      logger.info(`交易额度 < 交易精度`);
      return false;
    }
    logger.info(`基础货币[${pairAFrom}],交易额度: ${buyMoney}`)
